...
console.log('[HydraHD] module script loaded (Beta quality collapse + English/Spanish only)');
...
// This is an English + Spanish module: every other audio language (Italian,
// German, Portuguese, ...) is dropped everywhere it could surface — Beta
// per-language sources AND the VixSrc/Hydra2 audio-track siblings. English is
// the default/original for every host this module resolves, so an empty or
// unknown code counts as English and is kept.
const ALLOWED_AUDIO_LANGS = { eng: true, en: true, spa: true, es: true };
function isAllowedAudioLang(code) {
    const k = normalizeAudioLangCode(code);
    if (!k) return true;                         // no code => English original
    return ALLOWED_AUDIO_LANGS[k] === true;
}

// Decide a Beta entry's on-screen quality tier. Videasy hands each source its
// own quality string; when it's a real tier ("1080p", "2160p", "4K") use it,
// when it's "auto"/a host name it's a master playlist. If the field is unusable
// we dig a resolution out of the URL, which Videasy writes as either a filename
// marker (-s1080p-) OR a path segment (/1080p/). Only when nothing resolves do
// we call it a master → "auto".
function betaQualityClass(label, url) {
    const raw = String(label || '').trim().toLowerCase();
    if (/^\d{3,4}p$/.test(raw)) return qualityFromHeight(parseInt(raw, 10)) || raw;
    if (raw === '4k' || raw === '2160p' || raw === 'uhd') return '4K';
    if (raw === '1440p' || raw === '2k' || raw === 'qhd') return '1440p';
    const m = String(url || '').match(/(?:^|[-\/_.])s?(\d{3,4})p(?:[-\/_.]|$)/i);
    if (m) return qualityFromHeight(parseInt(m[1], 10)) || (m[1] + 'p');
    return 'auto';                               // master playlist / unknown
}
    ...
    try {
        ...
        function scheduleProbe(st) {
            try {
                ...
                // Beta entries already carry their tier from the endpoint/URL; the
                // probe rebuilds titles in the keyless-host shape, which would
                // double the flag and language for a Beta row. Leave them alone.
                if (st.selfLabeled) return;
        ...
        function pushLanguageVariants(parent, extraLangs) {
            ...
            for (const al of extraLangs) {
                ...
                if (!isAllowedAudioLang(al.code)) continue;   // English/Spanish only
            ...
            try {
                ...
                await resolveVideasyLink(ctxInfo, function(sources) {
                    for (const s of sources) {
                        ...
                        if (!isAllowedAudioLang(code)) continue;   // English/Spanish only
                        // The endpoint hands each source its own quality string
                        // (langLabel): a tier like "1080p"/"4K", or "auto"/a host
                        // name for a master playlist. Trust it first; fall back to
                        // digging a resolution out of the URL (filename -s1080p-
                        // OR path /1080p/), and only when NOTHING resolves treat
                        // it as an unlabelled master → "auto".
                        const qCls = betaQualityClass(s.langLabel, s.streamUrl);
                        const isAuto = (qCls === 'auto');
                        let title = languageFlag(code) + ' Beta • ' + subtitleLanguageName(code) + ' • ' + qCls;
                        const pushedSt = pushStream(title, s.streamUrl, { 'Referer': 'https://player.videasy.to/' });
                        if (pushedSt) {
                            pushedSt.selfLabeled = true;           // keep URL/field label; probe must not rewrite it
                            if (pushedSt.quality === title) pushedSt.quality = isAuto ? 'auto' : qCls;
                            // Tag for the post-settle Beta collapse: group by audio
                            // language, keep best non-4K + 4K (+ master when present).
                            pushedSt.betaGroup = normalizeAudioLangCode(code) || 'eng';
                            pushedSt.betaKind = isAuto ? 'auto' : 'rendition';
                            added++;
                        }
                    }
        ...
        // ---- Beta quality collapse -----------------------------------------
        // Beta returns a separate URL per resolution (480/720/1080/4K), which
        // flooded the picker with near-duplicate rows. Collapse each Beta audio
        // language down to at most: the master/auto (if the CDN exposed one),
        // the single best non-4K rendition, and the 4K rendition.
        //
        // Rule (per language group):
        //   - always keep 4K
        //   - if an auto/master exists: keep auto + 4K, and ALSO keep the best
        //     non-4K only when it out-resolves the master (master tops at 720p
        //     but a standalone 1080p exists => show all three)
        //   - if no auto exists: keep best non-4K + 4K
        // "Quality" here is the measured class where the probe reached the
        // entry, else the filename class — 4K/1440p/1080p/720p/480p.
        (function collapseBeta() {
            // A master ("auto") sits above 1080p and below 4K for the purpose of
            // "does the standalone rendition out-resolve the master": most Beta
            // masters top out at 1080p, so a standalone 4K always survives and a
            // standalone 1080p is considered covered unless it clearly beats it.
            const AUTO_RANK = 1.5;               // between 1440p (1) and 1080p (2)
            const rankOf = function(st) {
                if (st && st.quality === 'auto') return AUTO_RANK;
                return Object.prototype.hasOwnProperty.call(QUALITY_RANK, st && st.quality)
                    ? QUALITY_RANK[st.quality] : 9;              // lower = better
            };
            const groups = {};
            for (const st of streams) {
                if (!st || !st.betaGroup) continue;
                (groups[st.betaGroup] = groups[st.betaGroup] || []).push(st);
            }
            const drop = {};                                     // streamUrl -> true
            for (const lang in groups) {
                if (!Object.prototype.hasOwnProperty.call(groups, lang)) continue;
                const entries = groups[lang];
                if (entries.length <= 1) continue;               // nothing to collapse
                const keep = new Set();
                const autos = entries.filter(function(s) { return s.betaKind === 'auto'; });
                const fourK = entries.filter(function(s) { return s.quality === '4K'; });
                const nonFourK = entries.filter(function(s) { return s.betaKind === 'rendition' && s.quality !== '4K'; });
                fourK.forEach(function(s) { keep.add(s); });
                // Best (lowest rank number) non-4K rendition.
                let bestNon = null;
                for (const s of nonFourK) { if (!bestNon || rankOf(s) < rankOf(bestNon)) bestNon = s; }
                if (autos.length) {
                    // Keep every auto/master (usually one).
                    autos.forEach(function(s) { keep.add(s); });
                    // Best auto rank to compare the standalone rendition against.
                    let bestAuto = null;
                    for (const s of autos) { if (!bestAuto || rankOf(s) < rankOf(bestAuto)) bestAuto = s; }
                    // Keep the standalone only if it genuinely out-resolves the
                    // master (strictly better rank). Equal/worse => master covers it.
                    if (bestNon && rankOf(bestNon) < rankOf(bestAuto)) keep.add(bestNon);
                } else if (bestNon) {
                    keep.add(bestNon);
                }
                for (const s of entries) { if (!keep.has(s)) drop[s.streamUrl] = true; }
            }
            if (Object.keys(drop).length) {
                for (let i = streams.length - 1; i >= 0; i--) {
                    if (drop[streams[i].streamUrl]) streams.splice(i, 1);
                }
                console.log('[HydraHD] Beta collapse removed=' + Object.keys(drop).length + ' remaining=' + streams.length);
            }
        })();

        streams.sort(function(a, b) {
            const rq = function(x) {
                if (x && x.quality === 'auto') return 1.5;   // master ≈ high-1080p
                return Object.prototype.hasOwnProperty.call(QUALITY_RANK, x.quality) ? QUALITY_RANK[x.quality] : 9;
            };
            return rq(a) - rq(b);
        });
...
// English + Spanish module, so only endpoints that can yield those audio
// languages are worth fetching. cdn = English (incl. 4K), lamovie = Spanish.
// Dropped: meine (German), superflix (Portuguese) — always filtered out
// downstream — and hdmovie, which is mixed-language but measured 10-13s per
// fetch while almost never adding an English/Spanish dub cdn/lamovie lacked;
// its cost dominated Beta resolution time.
const VIDEASY_ENDPOINTS = ['cdn', 'lamovie'];
const VIDEASY_LANG = {
    ...
    for (const ep of VIDEASY_ENDPOINTS) {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
          try {
            ...
            const epLang = { lamovie: 'spa', cdn: 'eng' }[ep] || '';
