...
async function extractEpisodes(url) {
    try {
        ...
        // DUB-only: availableEpisodesDetail.dub is the authoritative dub list.
        const detail = await gql(`{show(_id:${JSON.stringify(showId)}){availableEpisodesDetail}}`);
        const dubList = ((((detail || {}).show || {}).availableEpisodesDetail) || {}).dub || [];
        const dubSet = new Set(dubList.map(n => String(n)));
