...
async function extractStreamUrl(url) {
    try {
        console.log("[Animepahe-DUB] Fetching episode page: " + url);
        ...
        console.log("[Animepahe-DUB] Resolution buttons found: " + buttons.length);

        if (buttons.length === 0) {
            console.warn("[Animepahe-DUB] No resolution buttons, using fallback method.");
        ...
        const deepUnpack = (source) => {
            ...
            while (/eval\(function\(p,a,c,k,e,d\)/.test(decoded) && safety < 5) {
                try {
                    ...
                    console.warn("[Animepahe-DUB] Unpack error at depth " + safety + ": " + e.message);
                    break;
                }
            }
            return decoded;
        ...
        const streamPromises = dubButtons.map(async (btn) => {
            const kwikUrl = btn.src;
            const title = btn.resolution === "auto" || btn.resolution === "master"
                ? "Auto"
                : btn.resolution + "p";
            console.log("[Animepahe-DUB] Fetching Kwik: " + kwikUrl + " | " + title);
            ...
            try {
                ...
                console.log("[Animepahe-DUB] Kwik response (first 300): " + html.substring(0, 300));
                ...
                console.log("[Animepahe-DUB] Eval blocks found: " + evalBlocks.length);
                ...
                if (evalBlocks.length === 0) {
                    ...
                    if (scriptMatch) {
                        ...
                        if (unpacked) {
                            ...
                            if (urlMatch) {
                                ...
                                console.log("[Animepahe-DUB] Extracted via fallback script: " + hlsUrl);
                                return {
                                    title: title,
                                    streamUrl: hlsUrl,
                    ...
                    console.warn("[Animepahe-DUB] No eval blocks or script fallback for " + kwikUrl);
                    return null;
                }
                ...
                for (const block of evalBlocks) {
                    try {
                        ...
                        console.log("[Animepahe-DUB] Unpacked snippet: " + unpacked.substring(0, 150));
                        ...
                        console.warn("[Animepahe-DUB] Failed to unpack block: " + e.message);
                    }
                }

                if (!hlsUrl) {
                    console.warn("[Animepahe-DUB] No HLS URL found in any block for " + kwikUrl);
                    return null;
                }

                hlsUrl = hlsUrl.replace(/\\+$/, '');
                hlsUrl = hlsUrl.replace("/stream/", "/hls/").replace("uwu.m3u8", "owo.m3u8");
                console.log("[Animepahe-DUB] Extracted: " + title + " | " + hlsUrl);
                return {
                    title: title,
                    streamUrl: hlsUrl,
                    headers: { "Referer": "https://kwik.cx/", "Origin": "https://kwik.cx" }
                ...
                console.warn("[Animepahe-DUB] Error fetching Kwik " + kwikUrl + ": " + e.message);
                return null;
            }
        });
        ...
        console.log("[Animepahe-DUB] Total successful streams: " + streams.length);
        ...
        console.log("[Animepahe-DUB] Fetch error in extractStreamUrl: " + err);
        return JSON.stringify({ streams: [], subtitle: "" });
    }
}
