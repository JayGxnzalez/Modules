...
async function extractStreamUrl(ID) {
    try {
        ...
        if (streamJson && streamJson.status_code === "200" && streamJson.data && streamJson.data.stream_urls) {
            const is4K = detectQuality(streamJson.data.file_name || streamJson.data.title || "") === "2160p";

            streamJson.data.stream_urls.forEach((url, index) => {
                streamObjects.push({
                    title: is4K ? `Server ${index + 1} - 4K` : `Server ${index + 1}`,
                    streamUrl: url,
                    headers: {
                        "Referer": "https://nextgencloudfabric.com/",
                        "Origin": "https://nextgencloudfabric.com",
            ...
            if (streamJson.default_subs && Array.isArray(streamJson.default_subs)) {
                allSubtitles = streamJson.default_subs
                    .filter(sub => sub && sub.url)
                    .map(sub => ({
                        url: sub.url,
                        label: sub.language || sub.lang || sub.label || "Subtitle",
                        kind: "subtitles",
                        headers: {}
                    }));
        ...
        console.log(`[IMDb] Return summary streams=${streamObjects.length} subs=${allSubtitles.length} default=${subtitleUrl ? "yes" : "none"}`);
