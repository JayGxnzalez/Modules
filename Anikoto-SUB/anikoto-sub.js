...
// Hardsub (HSUB) embeds return no tracks[]; softsub (SUB) embeds do, and
// that's where allSubtitles + the default subtitle come from.
...
// Everything comes directly from anikototv.to, same pattern as the DUB
// module: search the real site, use whatever's actually there. No external
// ID service (AniList/MAL) involved anywhere in this file.
// ══════════════════════════════════════════════════════════════════
class Anikoto {
    ...
    static async getEpisodes(showId) {
        ...
        while ((m = re.exec(html)) !== null) {
            ...
            if (!num || !ids) continue;
            episodes.push({ num: parseInt(num, 10), hasSub: g("sub") === "1", hasDub: g("dub") === "1", ids });
        }

        console.log("[Anikoto] Parsed " + episodes.length + " episodes for showId " + showId + ", sub count: " + episodes.filter(e => e.hasSub).length);
        return episodes;
    }

    // Confirmed live: HTML with sub/dub <div class="type"> blocks, each
    // server <li> carrying a data-link-id token.
    static async getServerList(idsToken, audioTypes) {
        ...
        // Anikoto renders each audio track as its own <div class="type"
        // data-type="…"> row: sub, hsub (hardsub), dub. Capture every
        // requested type and tag each server with the block it came from,
        // so the same provider (e.g. VidPlay) can legitimately appear under
        // both sub and hsub.
        const items = [];
        const typeRe = /<div class="type" data-type="(sub|hsub|dub)">([\s\S]*?)<\/ul>\s*<\/div>/g;
        let typeM;
        while ((typeM = typeRe.exec(html)) !== null) {
            const type = typeM[1];
            if (!audioTypes.includes(type)) continue;
            for (const li of typeM[2].matchAll(/<li\s+([^>]*data-link-id[^>]*)>([\s\S]*?)<\/li>/g)) {
                ...
                if (linkId) items.push({ linkId, name, type });
            }
        }
        console.log("[Anikoto] Found " + items.length + " server(s) across [" + audioTypes.join(", ") + "]");
        return items;
    }
    ...
    // type param. The variant is the last path segment of the embed URL
    // (…/sub or …/hsub); vidtube needs it in type=, megaplay/vidwish bake
    // it into the data-id so no type param is passed there. hsub embeds
    // occasionally serve under the plain "sub" type, so that's tried as a
    // fallback. Response shape (sources.file, tracks[]) is identical across
    // all three, so buildStreamResult() is unchanged.
    static async extractFromEmbed(embedUrl) {
        const host = [MEGAPLAY, VIDWISH, VIDTUBE].find(h => embedUrl.includes(h.replace("https://", "")));
        if (!host) {
            console.warn("[Anikoto] Unrecognized sub server host — no extractor for: " + embedUrl);
            return null;
        }
        ...
        // (HD-1 = tcdn, HD-2 = bcdn), so dropping it collapses every server
        // onto the plain Vidstream stream — which is often the broken one.
        // Character class matches the site's own sanitiser.
        const sParam = (String(embedUrl).match(/[?&]s=([A-Za-z0-9_-]+)/) || [])[1] || "";
        const sSuffix = sParam ? "&s=" + encodeURIComponent(sParam) : "";

        if (host === VIDTUBE) {
            const variant = (embedUrl.split("?")[0].match(/\/(hsub|sub|dub)$/i)?.[1] || "sub").toLowerCase();
            let data = await fetchVidtubeSources(fileId, variant, sSuffix);
            if (!data?.sources?.file && !data?.enc && variant === "hsub") data = await fetchVidtubeSources(fileId, "sub", sSuffix);
            if (!data?.sources?.file && !data?.enc) return null;
            return buildStreamResult(data, host + "/");
        }

        const sourcesUrl = `${host}/stream/getSources?id=${fileId}&id=${fileId}` + sSuffix;
        ...
        // Accept either shape: v2.8 responses only carry `enc`.
...
async function fetchVidtubeSources(fileId, type, sSuffix) {
    const url = `${VIDTUBE}/stream/getSourcesNew?id=${fileId}&type=${type}&id=${fileId}&type=${type}` + (sSuffix || "");
    const resp = await soraFetch(url, {
        headers: { "Referer": VIDTUBE + "/", "User-Agent": UA, "X-Requested-With": "XMLHttpRequest" }
    });
    if (!resp || resp.status !== 200 || typeof resp.json !== "function") return null;
...
// Only sub-available episodes are listed (data-sub="1" on Anikoto's own
...
async function extractEpisodes(url) {
    try {
        ...
        const subOnly = episodesData.filter(ep => ep.hasSub);
        console.log("[extractEpisodes] " + subOnly.length + " of " + episodesData.length + " episodes have a sub");

        const sorted = subOnly.sort((a, b) => a.num - b.num);
...
async function extractStreamUrl(url) {
    try {
        ...
        const subServers = await Anikoto.getServerList(idsToken, ["sub", "hsub"]);
        if (subServers.length === 0) {
            console.warn("[extractStreamUrl] No sub servers available for this episode");
        ...
        // Resolve every server in parallel. Classification is by the
        // resolved embed URL (…/hsub vs …/sub), which is authoritative even
        // when the same provider appears under both Anikoto type blocks.
        const resolved = await Promise.allSettled(subServers.map(async (server) => {
            ...
            const isHard = /\/hsub(?:$|\?)/i.test(embedUrl);
            return { isHard, name: cleanLabel(server.name), ...streamData };
        }));

        const hsub = [], sub = [];
        for (const r of resolved) {
            if (r.status !== "fulfilled" || !r.value) continue;
            (r.value.isHard ? hsub : sub).push(r.value);
        }
        ...
        const seenStream = {};
        const pushStream = (title, s) => {
            if (!s.streamUrl) return;
            const key = dedupeKey(s.streamUrl);
            if (seenStream[key]) return;
            seenStream[key] = true;
            streams.push({ title, streamUrl: s.streamUrl, headers: s.headers });
        };

        // HSUB pinned to the top so it doesn't get lost in the softsub list.
        // In practice there's a single hardsub source; if more ever appear,
        // the first working one wins. Hardsub is burned in, so it carries no
        // external subtitle tracks — nothing to aggregate from it.
        if (hsub.length) pushStream("HSUB", hsub[0]);

        // Then every softsub server, keeping its own name: "SUB - Vidstream",
        // "SUB - HD-1", etc. Subtitles/allSubtitles come from these only.
        for (const s of sub) {
            pushStream("SUB - " + s.name, s);
        ...
        if (streams.length === 0) {
            console.warn("[extractStreamUrl] Sub servers found but none resolved to a working stream");
            return JSON.stringify({ streams: [], subtitles: "", subtitlesHeaders: {}, allSubtitles: [] });
        }
        ...
        console.log("[extractStreamUrl] " + streams.length + " stream(s): " + streams.map(s => s.title).join(", "));
        return out;
    } catch (error) {
